...
# Plan:
# - Move bezier code to seperate file
# - Good maintainable code structure
# - Make helper functions and extract good stuff from rocket
# - Start experimenting
...
class RobotsSketch(vsketch.SketchClass):
    ...
    body_circle_radius_max = vsketch.Param(2.0, min_value=0)
    body_circle_radius_min = vsketch.Param(1.0, min_value=0)
    ...
    body_bullet_radius_max = vsketch.Param(2.0, min_value=0)
    body_bullet_radius_min = vsketch.Param(1.0, min_value=0)
    body_bullet_lower_height_max = vsketch.Param(2.0, min_value=0)
    body_bullet_lower_height_min = vsketch.Param(1.0, min_value=0)
    ...
    def draw_body(self, vsk, debug=False):
        ...
        if body_choice == enum_type_to_int(self.body_types.RECT):
            ...
            lower_height = np.random.uniform(self.body_bullet_lower_height_min, self.body_bullet_lower_height_max)
            self.body_lower_height = lower_height
            
            draw_bullet_body(vsk, self.body_width, arc_height, lower_height)
            
        # Eyes:
        vsk.fill(1)
        with vsk.pushMatrix():
            vsk.translate(0.2 * self.body_width, 0)    
            vsk.ellipse(0, 0, 0.05, 0.075)
        with vsk.pushMatrix():
            vsk.translate(-0.2 * self.body_width, 0)    
            vsk.ellipse(0, 0, 0.05, 0.075)
        vsk.noFill()
        ...
        with vsk.pushMatrix():
            ...
            draw_bezier(vsk, np.array([-0.1, 0]), np.array([0.1, 0]), np.array([-0.1, 0.2]), np.array([0.1, 0.2]), debug=debug)
    ...
    def draw_robot(self, vsk, debug=False):
        ...
        # with vsk.pushMatrix():
        #     vsk.translate(0.3 * body_width, -0.5 * body_height - 0.5 * 0.15)
        #     vsk.sketch(antenna_sketch)
        #     vsk.translate(-2 * 0.3 * body_width, 0)
        #     vsk.sketch(antenna_sketch)
